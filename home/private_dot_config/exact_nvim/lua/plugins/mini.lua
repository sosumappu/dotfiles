    ...
    config = function()
      ...
      for _, word in ipairs({
        ...
        comments[w] = extra.gen_highlighter.words(
          ...
          highlight_if_ts_capture("comment", string.format("MiniHipatterns%s", hl:sub(1, 1):upper() .. hl:sub(2)))
        )
      end
      ...
      if vim.fn.argc(-1) == 0 then
        vim.api.nvim_create_autocmd({ "VimEnter", "FocusGained" }, {
          ...
          callback = function()
            ...
            if sessions.detected[session_name] and string.find(vim.v.this_session, session_name, 1, true) == nil then
              return
            else
