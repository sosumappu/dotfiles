    ...
    config = function(_, opts)
        ...
        if LazyVim.has("mason.nvim") then
            ...
            if opts.dap and LazyVim.has("nvim-dap") and mason_registry.is_installed("java-debug-adapter") then
                local debug_bundle = get_pkg_path("java-debug-adapter")
                bundles = vim.fn.glob(debug_bundle .. "/extension/server/com.microsoft.java.debug.plugin-*.jar",
                    true, true)
                -- java-test also depends on java-debug-adapter.
                if opts.test and mason_registry.is_installed("java-test") then
                    local test_bundle = get_pkg_path("java-test")
                    vim.list_extend(bundles, vim.fn.glob(test_bundle .. "/extension/server/*.jar", true, true))
                end
            end
        end
