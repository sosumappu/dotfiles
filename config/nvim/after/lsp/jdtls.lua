local bundles = {}
local bundles_specs = {
	{ name = "JAVA_TEST_PATH", path = "/share/vscode/extension/server/*.jar" },
	{ name = "JAVA_DEBUG_PATH", path = "/share/vscode/extension/server/com.microsoft.java.debug.plugin-*.jar" },
}

for _, spec in ipairs(bundles_specs) do
	vim.list_extend(bundles, vim.fn.glob(os.getenv(spec.name) .. spec.path, true, true))
end
