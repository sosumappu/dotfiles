...
function M.get_icon(icon_name)
  local ICONS = {
    paste = "⍴",
    spell = "✎",
    branch = vim.env.PROMPT_GIT_BRANCH ~= nil and vim.fn.trim(vim.env.PROMPT_GIT_BRANCH) or "",
    error = "×",
    info = "𝒾",
    warn = "⚐",
    hint = "•",
...
function M.is_localhost()
  return vim.fn.hostname() == "localhost"
end
...
function M.web_roots(_, on_dir)
  on_dir(vim.fs.root(0, { "package.json", ".git", vim.api.nvim_buf_get_name(0) }))
end
